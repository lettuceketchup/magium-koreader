...
- **Status:** **IMPLEMENTATION COMPLETE — the port is feature-complete
  ...
  ([ADR-008](../decisions/ADR-008-license-and-distribution.md))** — port code
  ...
  **Phase I complete** ([spec](../specs/2026-08-31-plugin-architecture-and-phase-i.md)
  + [ADR-004](../decisions/ADR-004-plugin-internal-architecture.md); on-device
  ...
  **Phase II implemented 2026-09-01** ([spec](../specs/2026-09-01-phase-ii-full-corpus-and-navigation.md);
  ...
  routed, back/history stack cut ([ADR-006](../decisions/ADR-006-no-scene-back-navigation.md)).
...
[ADR-002](../decisions/ADR-002-porting-approach.md); full comparison in
...
fallback ([`04` §4](research/04-constraints-budget.md#4-runtime-parsing-vs-build-time-preprocessing-34))
...
itself — see [`06` §3](research/06-approach-comparison.md#3-blocking-open-questions-63) /
[`07`'s blocking-status note](research/07-risks-open-questions.md#blocking-status-after-phase-6).
**OQ-004** (does the family's permission extend to a further port?) and
**OQ-005** (license) are **closed** (Phase 7, 2026-09-08,
[ADR-008](../decisions/ADR-008-license-and-distribution.md), superseding the
...
is fully permitted — see [`08-licensing.md`](research/08-licensing.md).
**Phase 8 (roadmap/effort) is now done** — see
[`09-roadmap-effort.md`](research/09-roadmap-effort.md): a phased
...
green light* ([`04` §5](research/04-constraints-budget.md#5-go--no-go-verdict)),
...
| 5 | **Target device: Kindle Paperwhite 12th gen (2024), 16 GB** (Amazon.in B0DKTZ6592). Confirmed on-device: FW **Kindle 5.19.5**, **956.9 MB RAM** (~497 MB available), 10.6 GB free storage, KOReader **v2026.07.1** release (`kindlehf`) idling at ~33 MB RSS. | high | [`00-overview.md`](research/00-overview.md) — on-device |
| 6 | Story scale (English): **54 files, 7.5 MB, 2159 scenes, 4880 paragraphs, 3734 choices**. Fully parsed ≈ 17 MB in V8; 8.16 MB serialized. | high | measured — [`01`](research/01-magium-analysis.md) §11, [`reference/magium-dev-notes.md`](../../reference/magium-dev-notes.md) |
...
Tracked in [`docs/research/07-risks-open-questions.md`](research/07-risks-open-questions.md)
(OQ-001 … OQ-013). **Every OQ is now closed.** OQ-004 (redistribution
permission) + OQ-005 (license) closed in Phase 7, 2026-09-08
([ADR-008](../decisions/ADR-008-license-and-distribution.md)): story text is
...
note](research/07-risks-open-questions.md#blocking-status-after-phase-6).
...
**Phase 8** ([`09-roadmap-effort.md`](research/09-roadmap-effort.md)):
...
ordered mitigation list ([`04` §3 row 4](research/04-constraints-budget.md#3-budget-table-33)).
...
- [ADR-001](../decisions/ADR-001-research-dossier-layout.md) — research organized as a modular dossier (not a single report or a wiki).
- [ADR-002](../decisions/ADR-002-porting-approach.md) — port Magium as a standalone KOReader plugin with a Lua reimplementation of the engine (candidate A), over extending an existing plugin (B), converting to Twine/Ink + an existing player (C), or a build-time hybrid (D).
- [ADR-003](../decisions/ADR-003-defer-licensing-distribution.md) — *(superseded by ADR-008)* deferred licensing work while the project was personal-use-only.
- [ADR-004](../decisions/ADR-004-plugin-internal-architecture.md) — plugin internal architecture: three-layer (engine-pure / ui / save), a custom fullscreen paginated reading widget (resolves OQ-013), choices rendered as the final page.
- [ADR-005](../decisions/ADR-005-debug-trace-toggle.md) — the debug action-trace is a runtime menu toggle, not a build variant.
- [ADR-006](../decisions/ADR-006-no-scene-back-navigation.md) — **no** in-game "back one scene" / history stack: `magium-dev` has none, and parity is the standard. Overrides the roadmap's Phase II "back/history stack" line.
- [ADR-007](../decisions/ADR-007-saves-scope.md) — save-slot scope: 50 slots, delete yes, import/export & rename cut.
- [ADR-008](../decisions/ADR-008-license-and-distribution.md) — license the port **AGPL-3.0-or-later** (matching KOReader), release it freely and non-commercially, keep the bundled Magium story text under **CC BY 4.0**. Supersedes ADR-003.
...
**Phase II implemented 2026-09-01** ([spec](../specs/2026-09-01-phase-ii-full-corpus-and-navigation.md),
[plan](superpowers/plans/2026-09-01-phase-ii-full-corpus-and-navigation.md),
