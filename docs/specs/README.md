...
| [2026-09-08-phase-vii-localization.md](2026-09-08-phase-vii-localization.md) | draft — awaiting owner review | **Phase VII**: bundle `data/fr/` (verified structural drop-in — scene IDs == en); `magium_lang` `G_reader_settings` key + a Language row in Settings; live-reload switch (`current_lang`/`ensure_locale`, re-parse + re-render in place); chrome stays English except what `ui.json` covers ([ADR-008](../decisions/ADR-008-localization-scope.md)). |
