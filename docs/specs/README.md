...
| [2026-09-07-phase-vi-settings.md](2026-09-07-phase-vi-settings.md) | stable — merged 2026-09-07 | **Phase VI**: scoping pass (theme/language not ported); cheat mode + reader text-size presets (`magium_prose_size`); custom-reader viewport robustness — choices scroll on overflow, `prose_height` floor, `test-ui-matrix` across 4 device profiles; fixed a pre-existing reopen bug (store reloaded from disk on every `_reopenReader`). |
| [2026-09-08-phase-viii-polish.md](2026-09-08-phase-viii-polish.md) | stable — merged 2026-09-08, tag `v1.0` | **Phase VIII** (final): measurement-driven polish. OQ-011 closed on a dev bench (`condition_perf_spec.lua`, 2.6 ms/render x86); GC downgraded to a post-release watch (owner can't run long sessions); the only device work is a ≤5-min e-ink feel pass on `ui/refresh.lua` (OQ-007). Full-corpus QA via `oracle-corpus`; `INSTALL.md` + `v1.0` tag; project declared feature-complete. |
