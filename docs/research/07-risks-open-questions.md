...
- **Last updated:** 2026-08-31 (Phase 3: no new OQs; OQ-001 further downgraded, OQ-011 gains a mitigation list — see [`04`](04-constraints-budget.md))
- **Phase:** all
...
| OQ-007 | E-ink refresh feel for a "tap choice → new page of prose" loop — acceptable or sluggish/ghosting? | Core UX viability. | spike A | yes | open | Platform read ([`03` §6](03-koreader-platform.md#6-e-ink-specifics-26)): the tools exist — `"ui"` (no-flash) for scene swaps/scrolling, occasional `"full"` to de-ghost, MTK fast-mode is force-enabled on `KindlePaperWhite6` so `"ui"` stays quick. Actual latency + ghosting judgment still needs the device (spike A). |
...
| OQ-012 | What is the least-friction KOReader dev loop on Windows (the owner's OS)? The `kodev` emulator is Linux/macOS-only; options are WSL2 + X server, the `koreader/virdevenv` Docker image, extracting a Linux AppImage for pure-Lua work, or on-device-only (USB copy + `crash.log`). | Sets the iteration speed and therefore the effort estimate for all KOReader-facing work (Phase 8). | spike A / KOReader Docker docs | no — mitigable | **resolved (2026-08-31):** **WSL2 / Ubuntu 24.04** — `./kodev build` + `./kodev run` working; **WSLg** gives a display with no X server (SDL `x11` driver). Needed ninja ≥1.13.2 + GNU make ≥4.4 in `/usr/local/bin` (Ubuntu 24.04 ships 1.11.1 / 4.3 → recursive-make thirdparty builds fail `read jobs pipe: Bad file descriptor`). Reproducible: [`reference/setup-koreader-wsl.sh`](../../reference/setup-koreader-wsl.sh); notes in [`reference/koreader-notes.md`](../../reference/koreader-notes.md). Build ~7 min. |
